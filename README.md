...
For the GUI and newer Python versions, see the "GUI batch inference" section
below and use `requirements-gui.txt`.
...
## GUI batch inference
This repo includes a simple GUI for batch transcription: `transcribe_gui.py`.

Steps:
1) Put your checkpoint in `Models/` (auto-detected) or select it manually. Checkpoints are not included in this repo.
2) Run the GUI: `python transcribe_gui.py`
3) Add files or folders, choose device, and run.

Notes:
- Output defaults to the same folder as each input audio file (you can set a custom output folder).
- If a MIDI file already exists, it will auto-rename to `_1`, `_2`, etc.
- MP3 decoding requires FFmpeg available on PATH.

Dependencies for the GUI on modern Python:
```
pip install -r requirements-gui.txt
```
Install PyTorch separately (CUDA or CPU) from https://pytorch.org/get-started/locally/
